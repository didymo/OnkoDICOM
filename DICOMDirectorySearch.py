...
from src.Model.DICOMStructure import DICOMStructure, Patient, Study, Series, \
                                     Image
...
def get_dicom_structure(path, interrupt_flag, progress_callback):
    ...
    for root, dirs, files in os.walk(path, topdown=True):
        ...
        for file in files:
            ...
            # represents ALL files inside the selected directory, not
            ...
            try:
                ...
                if "SOPInstanceUID" in dicom_file and \
                        "SOPClassUID" in dicom_file and \
                        "Modality" in dicom_file:
                    new_image = Image(file_path, dicom_file.SOPInstanceUID,
                                      dicom_file.SOPClassUID,
                                      dicom_file.Modality)
                    if not dicom_structure.has_patient(patient_id):
                        # TODO there is definitely a more efficient way
                        #  of doing this
