...
NOTE REQUIRES: 2 directories of the same patient in the test data, if there isn't it will skip these tests
"""
...
def find_first_two_image_dirs(testdata_path):
    ...
    If only one is found, disables moving image tests and prevents test data modification.
    """
    image_dirs = []
    for subdir in sorted(testdata_path.iterdir()):
        if subdir.is_dir():
            ...
            if len(image_dirs) == 2:
                break
    if len(image_dirs) == 1:
        # Only one directory: skip tests that require two, and prevent test data modification
        pytest.skip("Only one image directory found in test/testdata; skipping fusion tests that require two image sets.")
    if not image_dirs:
        pytest.skip("No image series found in test/testdata; skipping fusion tests.")
...
def fusion_test_data():
    ...
    # Remove any RTSS from the dicts if present, but do NOT modify or delete any files on disk
    read_data_dict = {k: v for k, v in read_data_dict.items() if k != "rtss"}
    file_names_dict = {k: v for k, v in file_names_dict.items() if k != "rtss"}
