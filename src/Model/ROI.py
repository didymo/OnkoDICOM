...
def create_roi(rtss, roi_name, roi_coordinates, data_set,
    ...
    # This is for adding a new slice to an already existing ROI. For Future
    # Development. Check to see if the ROI already exists
...
def get_raw_contour_data(rtss):
    ...
    :return: dict_roi, a dictionary of ROI contours; dict_num_points, number
    of points of contours.
    """
...
def calculate_pixels(pixlut, contour, prone=False, feet_first=False):
    """
    ...
    :param feet_first: label of feetfirst or head first
    ...
    # if (not prone and not feet_first):
    ...
    np_x = np.array(pixlut[0])
    np_y = np.array(pixlut[1])
    if not feet_first and not prone:
        for i in range(0, len(contour), 3):
            con_x = contour[i]
            con_y = contour[i + 1]
            x = np.argmax(np_x > con_x)
    ...
    # for i in range(0, len(contour), 3):
    #     for x, x_val in enumerate(pixlut[0]):
    #         if (x_val > contour[i] and not prone and not feet_first):
    #             break
    #         elif (x_val < contour[i]):
    #             if feet_first or prone:
    #                 break
    #     for y, y_val in enumerate(pixlut[1]):
...
def pixel_to_rcs(pixlut, x, y):
    """
    :param pixlut: Transformation matrix
    :param x: Pixel X value (greater than 0, less than the slice's Columns data
    element)
    :param y: Pixel Y value (greater than 0, less than the slice's Rows data
    element)
    :return: The pixel coordinate converted to an RCS point as set by the image
    slice.
    """
...
def get_contour_pixel(
    ...
    :param prone: label of prone
    :param feet_first: label of feet_first or head first
    ...
    for roi in roi_selected:
        ...
        for i in range(number_of_contours):
            contour_pixels = calculate_pixels(
                pixlut, raw_contours[curr_slice][i], prone, feet_first
            )
...
def create_initial_rtss_from_ct(img_ds: pydicom.dataset.Dataset,
    ...
    pydicom.dataset.Dataset
        the half-baked RT SS, ready for Structure Set ROI Sequence, ROI Contour
        Sequence, and RT ROI Observations Sequence
    ...
    rt_ref_series_sequence_item.SeriesInstanceUID = \
