...
class UIAddOnOptions(object):
    ...
    def __init__(self):
        ...
        # The following can be used for future implementation of
        # creating ROIs from ISO dose.
        # self.table_roi = None
    ...
    def setup_ui(self, add_on_options, roi_line, roi_opacity, iso_line,
        ...
        add_on_options.setWindowTitle(_translate(
        ...
        self.line_fill_options = \
            LineFillOptions(self, roi_line, roi_opacity,
    ...
    def set_layout(self):
        ...
        self.layout.addWidget(self.tree_list, 0, 0)

        # Container including the table view and the option buttons for
        # each options
        ...
        self.option_layout.addWidget(self.change_default_directory_frame, 1, 0,
                                     1, 3)
    ...
    def init_user_options_header(self):
        ...
        self.option_title = QtWidgets.QLabel(self.widget)
        self.option_title.setGeometry(QtCore.QRect(290, 50, 281, 31))
        _translate = QtCore.QCoreApplication.translate
        self.option_title.setText(_translate("Add_On_Options", "User Options"))
    ...
    def init_tree_list(self):
        ...
        """
        self.tree_list = QtWidgets.QTreeView(self.widget)
        self.tree_list.setHeaderHidden(True)

        # Triggering the view change according to the row selected in the tree
        self.tree_list.clicked.connect(self.display)
        self.tree_list.setMaximumWidth(250)

    def display(self, index):
        """
        Function triggered when an item from the tree view on the left column
        is clicked.
        Change the display of the right view of the window in regards to the
        option chosen from the tree.
        """
        item = self.tree_list.selectedIndexes()[0]
        # Changes the title
        self.option_title.setText(item.model().itemFromIndex(index).text())
        # Changes the display
...
class WindowingOptions(object):
    ...
    def create_table_view(self):
        ...
        image_window_header = self.window.table_view.horizontalHeader()
        image_window_header.setSectionResizeMode(0,
                                                 QHeaderView.Stretch)
        image_window_header.setSectionResizeMode(1,
                                                 QHeaderView.ResizeToContents)
        image_window_header.setSectionResizeMode(2,
                                                 QHeaderView.Stretch)
        image_window_header.setSectionResizeMode(3,
                                                 QHeaderView.Stretch)
...
class StandardOrganOptions(object):
    ...
    def create_table_view(self):
        ...
        organ_names_header = self.window.table_organ.horizontalHeader()
        organ_names_header.setSectionResizeMode(0,
                                                QHeaderView.Stretch)
        organ_names_header.setSectionResizeMode(1,
                                                QHeaderView.ResizeToContents)
        organ_names_header.setSectionResizeMode(2,
                                                QHeaderView.Stretch)
        organ_names_header.setSectionResizeMode(3,
                                                QHeaderView.Stretch)
...
class StandardVolumeOptions(object):
    ...
    def create_table_view(self):
        ...
        standard_volume_names_header.setSectionResizeMode(0,
                                                          QHeaderView.Stretch)
        standard_volume_names_header.setSectionResizeMode(1,
                                                          QHeaderView.Stretch)
...
class PatientHashId(object):
    ...
    def create_table_view(self):
        """
        Create a table to hold all the patients and their hash
        that the software has anonymised.
        """
        ...
        patient_hash_id_header.setSectionResizeMode(0,
                                                    QHeaderView.Stretch)
        patient_hash_id_header.setSectionResizeMode(1,
                                                    QHeaderView.Stretch)
...
class LineFillOptions(object):
    ...
    def __init__(self, window_options, roi_line, roi_opacity, iso_line,
                 iso_opacity, line_width):
        """
        Create the components for the UI of Line and Fill options and set
        the layout.
        """
        self.window = window_options
    ...
    def set_layout(self):
        """
        Add the components into a layout and initialize the values according
        to the last configuration settings.
        """
...
class ChangeDefaultDirectory(object):
    ...
    def create_change_default_directory_frame(self):
        ...
        try:
            ...
            QMessageBox.critical(self.window, "Config file error",
                                 """Failed to access configuration file.
                              \nConfiguration file recreated with no default 
                              directory.\nPlease update your default 
                              directory.""")
