...
class UIOpenPatientWindow(object):
    ...
    def setup_ui(self, open_patient_window_instance):
        ...
        # Button doesn't show until a search commences
        ...
        self.open_patient_window_patient_open_actions_h_axis_box = \
            QHBoxLayout()
        self.open_patient_window_patient_open_actions_h_axis_box.setObjectName(
            "OpenPatientWindowPatientOpenActionsHorizontalBox")
        self.open_patient_window_patient_open_actions_h_axis_box.addStretch(1)
        ...
        self.open_patient_window_exit_button.setProperty(
            "QPushButtonClass", "fail-button")
        self.open_patient_window_patient_open_actions_h_axis_box.addWidget(
        ...
        # Create a widget to house all of the actions button for open
        # patient window
        ...
        # Set the vertical box fourth element, the tree view, to stretch out
        # as far as possible
        ...
        # Set the current stylesheet to the instance and connect it back to
        # the caller through slot
    ...
    def retranslate_ui(self, open_patient_window_instance):
        ...
        self.open_patient_directory_prompt.setText(_translate(
            ...
            "Choose the path of the folder containing DICOM files to load "
            "Patient's details:"))
        ...
        self.open_patient_directory_result_label.setText(
            "The selected directory(s) above will be opened in the "
            "OnkoDICOM program.")
        self.open_patient_window_stop_button.setText(_translate(
            "OpenPatientWindowInstance", "Stop Search"))
        self.open_patient_window_exit_button.setText(_translate(
            "OpenPatientWindowInstance", "Exit"))
    ...
    def scan_directory_for_patient(self):
        ...
        if self.filepath != "":
            ...
            # The interrupt flag is then un-set if a previous search has
            # been stopped.
    ...
    def search_progress(self, progress_update):
        ...
        self.open_patient_window_patients_tree.clear()
        self.open_patient_window_patients_tree.addTopLevelItem(
            QTreeWidgetItem(["Loading selected directory... (%s files "
                             "searched)" % progress_update]))

    def on_search_complete(self, dicom_structure):
        """
        Executes once the directory search is complete. :param
        dicom_structure: DICOMStructure object constructed by the directory
        search.
        """
        ...
        if dicom_structure is None:  # Will be None if function was interrupted
            return
    ...
    def tree_item_clicked(self, item, _):
        ...
        if item.checkState(0) == Qt.CheckState.Checked \
                and self.last_patient != selected_patient:
    ...
    def on_loaded(self, results):
        ...
        if results[0] is True:  # Will be NoneType if loading was interrupted.
            self.patient_info_initialized.emit(results[1])  # Emits progress
    ...
    def on_loading_error(self, error_code):
        ...
        if error_code == 0:
            QMessageBox.about(self.progress_window, "Unable to open selection",
                              "Selected files cannot be opened as they are "
                              "not a DICOM-RT set.")
            self.progress_window.close()
        elif error_code == 1:
            QMessageBox.about(self.progress_window, "Unable to open selection",
                              "Selected files cannot be opened as they "
                              "contain unsupported DICOM classes.")
            self.progress_window.close()
...
class UIOpenPatientWindowDragAndDropEvent(QLineEdit):
    ...
    def __init__(self, ui_open_patient_window_instance):
        super(UIOpenPatientWindowDragAndDropEvent, self).__init__()
        self.parent_window = ui_open_patient_window_instance
