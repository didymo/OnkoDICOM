...
from src.Model.ImageFusion import get_fused_pixmap, generate_comparison_colormix
...
class ImageFusionView(QtWidgets.QWidget):
    def __init__(self, roi_color=None, iso_color=None, format_metadata=False, slice_view="axial", color=False):
    ...
    def update_view(self, zoom_change=False, color=False):
        ...
        """
        if(color):
            self.image_display(color=color)
        else:
            self.image_display()
    ...
    def image_display(self, color=False):
        ...
        # pixmaps = self.patient_dict_container.get("pixel_values")
        # slider_id = self.slider.value()
        # image = pixmaps[slider_id]
        # label = QtWidgets.QGraphicsPixmapItem(image)
        # self.scene = QtWidgets.QGraphicsScene()
