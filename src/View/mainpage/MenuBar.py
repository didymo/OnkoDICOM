...
class MenuBar(object):
    ...
    def init_icons(self):
        ...
        self.iconOpen.addPixmap(
            QtGui.QPixmap(":/images/Icon/open_patient.png"),
            QtGui.QIcon.Normal,
            QtGui.QIcon.On,
        )
        self.iconAnonymize_and_Save = QtGui.QIcon()
        self.iconAnonymize_and_Save.addPixmap(
            QtGui.QPixmap(":/images/Icon/anonlock.png"),
            QtGui.QIcon.Normal,
            QtGui.QIcon.On,
        )
        self.iconZoom_In = QtGui.QIcon()
        self.iconZoom_In.addPixmap(
            QtGui.QPixmap(":/images/Icon/plus.png"), QtGui.QIcon.Normal, QtGui.QIcon.On
        )
        self.iconZoom_Out = QtGui.QIcon()
        self.iconZoom_Out.addPixmap(
            QtGui.QPixmap(":/images/Icon/minus.png"), QtGui.QIcon.Normal, QtGui.QIcon.On
        )
        self.iconWindowing = QtGui.QIcon()
        self.iconWindowing.addPixmap(
            QtGui.QPixmap(":/images/Icon/windowing.png"),
            QtGui.QIcon.Normal,
            QtGui.QIcon.On,
        )
        self.iconTransect = QtGui.QIcon()
        self.iconTransect.addPixmap(
    ...
    def create_actions(self):
        ...
        self.actionSave_as_Anonymous.triggered.connect(
            self.handlers.anonymization_handler
        )
...
class MenuHandler(object):
    ...
    def windowing_handler(self, state, text):
        ...
        self.main_window.pixmaps = get_pixmaps(
            self.main_window.pixel_values,
            self.main_window.window,
            self.main_window.level,
        )
    ...
    def anonymization_handler(self):
		...
		"""
        SaveReply = QtWidgets.QMessageBox.information(
            self.main_window,
            "Confirmation",
        ...
        if SaveReply == QtWidgets.QMessageBox.Yes:
            self.main_window.hashed_path = self.main_window.callClass.runAnonymization(
                self.main_window
            )
            # now that the radiomics data can just get copied across... maybe skip this?
            RadiomicsReply = QtWidgets.QMessageBox.information(
                self.main_window,
                "Confirmation",
            ...
            if RadiomicsReply == QtWidgets.QMessageBox.Yes:
                self.main_window.pyradi_trigger.emit(
                    self.main_window.path,
    ...
    def roi_delete_options_handler(self):
        ...
        if self.main_window.has_rtss:
            ...
            confirmation_dialog = QMessageBox.information(
                self.main_window,
                "Unable to open ROI Delete Window",
