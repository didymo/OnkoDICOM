...
class UIMainWindow:
    ...
    def setup_central_widget(self):
        ...
        # Left panel contains stuctures tab,
        # isodoses tab, and structure information
        ...
        if not patient_dict_container.has_modality("rtss") \
                and not patient_dict_container.has_modality("rtdose"):
        ...
        roi_color_dict = self.structures_tab.color_dict \
            if hasattr(self, 'structures_tab') else None
        iso_color_dict = self.isodoses_tab.color_dict \
            if hasattr(self, 'isodoses_tab') else None
        ...
        if patient_dict_container.has_modality("rtss") \
                and patient_dict_container.has_modality("rtdose"):
            self.dvh_tab = DVHTab()
        ...
        #
        # As they currently stand, they are given the right tab widget,
        # and make direct modifications to the tab. This class should be
        # refactored in the same way as the rest of the main window's
        # components, i.e. the Clinical Data should be a child of QWidget
        # that can exist independently of OnkoDICOM. This class would differ
        # from most other main window components in that rather than
        # interacting with the PatientDictContainer as its model, it would
        # use the patient's ClinicalData csv file as the model (which means
        # that the QWidget would theoretically easily exist outside
        # OnkoDICOM).
        #
