...
from PySide6.QtWidgets import QLabel, QVBoxLayout, QWidget, QHBoxLayout, \
    QPushButton, QTreeWidget, QTreeWidgetItem, QMessageBox, QAbstractItemView, \
    QSizePolicy
...
class UIDeleteROIWindow():
    def setup_ui(
            self, delete_roi_window_instance, regions_of_interest,
        ...
        # Create a vertical box for holding the 2 buttons for moving left
        # and right
        ...
        self.move_right_button.clicked.connect(
            self.move_right_button_on_clicked)
        self.move_right_button.setProperty(
        ...
        # Create a vertical box for holding the label and the tree view for
        # holding the ROIs that we are deleting
        ...
        self.delete_roi_window_cancel_button.setProperty("QPushButtonClass",
                                                         "fail-button")
        ...
        self.delete_roi_window_confirm_button.clicked.connect(
            self.confirm_button_on_clicked)
        self.delete_roi_window_confirm_button.setEnabled(False)
        self.delete_roi_window_confirm_button.setProperty("QPushButtonClass",
                                                          "success-button")
        self.delete_roi_window_action_buttons_box.addWidget(
    ...
    def retranslate_ui(self, delete_roi_window_instance):
        ...
        self.delete_roi_window_instruction.setText(
            _translate("DeleteRoiWindowInstruction",
                       "Move the Regions of Interest to be deleted to "
    ...
    def move_right_button_on_clicked(self):
        ...
        self.regions_of_interest_to_keep = \
            [x for x in
            self.regions_of_interest_to_keep if
    ...
    def on_rois_deleted(self, new_rtss):
        self.deleting_rois_structure_tuple.emit(
            (new_rtss, {"delete": self.regions_of_interest_to_delete}))
...
class DeleteROIProgressWindow(QtWidgets.QDialog):
    ...
    signal_roi_deleted = QtCore.Signal(
        pydicom.Dataset)  # Emits the new dataset
