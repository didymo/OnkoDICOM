...
# matplotlib.cobook.handle_exceptions = my_logger  # will be called with
...
class ClinicalDataForm(QtWidgets.QWidget, UiForm):
    ...
    # The following function performs the saving of the clinical data csv in
    # the directory
    ...
    def save_clinical_data(self):
        ...
        if len(message.strip()) == 0:
            ...
            if file_name.exists():
                ...
                for i in new_df.index:
                    if new_df.at[i, 'PID'] == self.pID:
                        ...
                        new_df.at[
                            i,
                            'DOB'] = self.ui.date_of_birth.date().toString(
                            "dd/MM/yyyy")
                        new_df.at[
                            i,
            ...
            save_reply = QMessageBox.information(self,
                                                 "Message",
                                                 "The Clinical Data "
                                                 "was saved "
                                                 "successfully in "
    ...
    def completer_fill(self, type, code):
        if type == 0:  # hist
    ...
    def editing_mode(self):
        # add the sensitive data of dates from the binary file date of birth
        # date of diagnosis date of last existence Create a dtype with the
        # binary data format and the desired column names
...
class ClinicalDataDisplay(QtWidgets.QWidget, UiCDDisplay):
    ...
    # This function will load the data from the csv into the software and
    # make it uneditable as it is only for display
    ...
    def edit_mode(self):
        ...
        if os.path.exists(resource_path('data/records.pkl')):
            ...
            button_reply = QMessageBox.warning(self,
                                               "Error Message",
                                               "The software has no previous "
                                               "records of this patient.\n "
                                               "If you wish, you can create "
                                               "a new clinical data file by "
                                               "\n "
...
class Transect(QtWidgets.QGraphicsScene):
    ...
    # This function performs the DDA algorithm that locates all the points
    # in the drawn line
    ...
    def _update_plot(self):
        if self.is_ROI_draw:
            if not self._points:
                ...
                self.roi_values.clear()
                for x in range(len(self.distances)):
                    if (self.distances[x] >= self.thresholds[0] and
                            self.distances[x] <= self.thresholds[1]):
    ...
    def _on_release(self, event):
        ...
        """
        if self.is_ROI_draw:
            if event.button == 1 and event.inaxes in [
                self._axes] and self._dragging_point:
                self._dragging_point = None
                self._update_plot()
...
# This is the main page Controller class that handles all the activity in
# the main page
...
class MainPageCallClass:
    ...
    def run_transect(self, main_window, tab_window, image_to_paint, dataset,
                     row_s, col_s, is_roi_draw=False):
        self.tab_ct = Transect(main_window, image_to_paint,
